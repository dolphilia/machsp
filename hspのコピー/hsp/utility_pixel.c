...
void copy_pixel_data(uint8_t *pixel_data_dest,
                     ...
                     int32_t copy_default_size_width, int32_t copy_default_size_height,
                     uint8_t color_red, uint8_t color_green, uint8_t color_blue, uint8_t color_alpha,
                     ...
                     int8_t blend_opacity)
{
    ...
    if (cx == 0 &&
        ...
        copy_point_x == 0 &&
        copy_point_y == 0 &&
        canvas_dest_size_width == canvas_source_size_width &&
        canvas_dest_size_height == canvas_source_size_height &&
        w == canvas_source_size_width &&
        h == canvas_source_size_height &&
        blend_mode == 0) {
    ...
    switch(blend_mode) {
        ...
        {
            for (int y=0; y<h; y++) {
                memcpy(&pixel_data_dest[cx*4+(y+cy)*canvas_dest_size_width*4],
                       &pixel_data_source[copy_point_x*4+y*canvas_source_size_width*4],
                       sizeof(unsigned char) * w * 4);
            }
            break;
        }
        case 2: //透明色付きコピー
        {
            for (int y=0; y<h; y++) {
                for (int x=0; x<w; x++) {
                    if(pixel_data_source[((x+copy_point_x)*4)+y*canvas_dest_size_width*4]!=0 &&
                       pixel_data_source[((x+copy_point_x)*4+1)+y*canvas_dest_size_width*4]!=0 &&
                       pixel_data_source[((x+copy_point_x)*4+2)+y*canvas_dest_size_width*4]!=0) {
        ...
        case 8: //透明色保護コピー
        {
            ...
            for (int y=0; y<h; y++) {
                for (int x=0; x<w; x++) {
                    ...
                    set_pixel_rgba_protect_alpha_fast(pixel_data_dest,
                                                      x+cx, y+cy,
                                                      color.red, color.green, color.blue, color.alpha,
                                                      canvas_dest_size_width, canvas_dest_size_height,
                                                      pixel_data_source[((x+copy_point_x)*4+3)+y*canvas_dest_size_width*4],
                                                      set_pixel_index);
                }
            }
            break;
