...
CToken::~CToken( void )
{
    if ( scnvbuf!=NULL ) InitSCNV(-1);    if ( tstack!=NULL ) { delete tstack; tstack = NULL; }
...
char *CToken::ExpandToken( char *str, int *type, int ppmode )
{
    ...
    if (a1==10) {							// Unix改行
        vs++;
        if (wrtbuf!=NULL) wrtbuf->PutStr( (char *)"\r\n" );
        *type = TK_EOL;
        return (char *)vs;
    }
    if (a1==13) {							// 改行
...
int CToken::ReplaceLineBuf( char *str1, char *str2, char *repl, int opt, MACDEF *macdef )
{
    ...
    *dummy = 0;
    ...
    while(1) {					// マクロ置き換え
        if ( wp==NULL ) break;
        if ( w>=linetmp ) { SetError((char *)"macro buffer overflow"); return 4; }
        a1=*wp++;if (a1==0) break;
        if (a1=='%') {
                ...
                switch( a1 ) {
                    ...
                    case 's':
                        ...
                        if ( !( 0 <= val && val <= macopt - 1 ) ) {
                            SetError((char *)"illegal macro parameter %s"); return 2;
                        }
                        w2 = mactmp;
            ...
            if ( type!=TK_NUM ) { SetError((char *)"macro parameter invalid"); return 1; }
            val--;
            if ( !( 0 <= val && val <= macopt - 1 ) ) {
                SetError((char *)"illegal macro parameter"); return 2;
            }
            p = prm[val]; endp = prme[val];
            if ( p==endp ) {				// 値省略時
    ...
    *w = 0;
    if ( last!=NULL ) {
        if ( w + strlen(last) + 1 >= linetmp ) { SetError((char *)"macro buffer overflow"); return 4; }
        strcpy( w, last );
    }
    return 0;
}
...
ppresult_t CToken::PP_Const( void )
{
    ...
    if (tstrcmp(word,"global")) {		// global macro
        if ( GetToken() != TK_OBJ ) {
            SetError( (char *)"bad global syntax" ); return PPRESULT_ERROR;
        }
        glmode=1;
        strcase( word );
    }
    ...
    if (valuetype != ConstType::Indeterminate) {
        if ( GetToken() != TK_OBJ ) {
            SetError( (char *)"bad #const syntax" ); return PPRESULT_ERROR;
        }
        strcase(word);
    }
    strcpy( keyword, word );
    ...
    if ( Calc(cres) ) return PPRESULT_ERROR;    //		AHT keyword check
    if ( ahtkeyword != NULL ) {
...
ppresult_t CToken::PP_Define( void )
{
    ...
    if (tstrcmp(word,"global")) {		// global macro
        if ( GetToken() != TK_OBJ ) {
            SetError( (char *)"bad macro syntax" ); return PPRESULT_ERROR;
        }
        glmode=1;
        strcase( word );
    }
    ...
    if ( a1==0 ) {					// no parameters
        ...
        wdata = CheckValidWord();        //		AHT keyword check
        if ( ahtkeyword != NULL ) {
            if ( ahtbuf != NULL ) {						// AHT出力時
                AHTPROP *prop;
                prop = ahtmodel->GetProperty( keyword );
    ...
    if ( a1 == 0 ) { SetError((char *)"macro contains no data"); return PPRESULT_ERROR; }
...
ppresult_t CToken::PP_Defcfunc( int mode )
{
    ...
    if ( i == TK_OBJ ) {
        ...
        if (tstrcmp(word,"local")) {		// local option
            if ( *mod == 0 ) { SetError((char *)"module name not found"); return PPRESULT_ERROR; }
            glmode = 1;
            i = GetToken();
        }
    ...
    if ( i != TK_OBJ ) { SetError((char *)"invalid func name"); return PPRESULT_ERROR; }
    ...
    if ( glmode ) AddModuleName( fixname );    if ( premode == LAB_TYPE_PP_PREMODFUNC ) {
    ...
    while(1) {        i = GetToken();
        ...
        if ( wp == NULL ) break;
        if ( i != TK_OBJ ) {
            SetError((char *)"invalid func param"); return PPRESULT_ERROR;
        }
        i = GetToken();
        if ( i == TK_OBJ ) {
            strcase2( word, fixname );
...
ppresult_t CToken::PP_Struct( void )
{
    ...
    while(1) {
        i = GetToken();
        if ( wp == NULL ) break;
        if ( i != TK_OBJ ) { SetError((char *)"invalid struct param"); return PPRESULT_ERROR; }
        wrtbuf->PutStr( word );
        wrtbuf->Put( ' ' );
        i = GetToken();
        if ( i != TK_OBJ ) { SetError((char *)"invalid struct param"); return PPRESULT_ERROR; }
        ...
        if ( wp == NULL ) break;
        if ( i != ',' ) {
            SetError((char *)"invalid struct param"); return PPRESULT_ERROR;
        }
        wrtbuf->Put( ',' );
    }
    wrtbuf->PutCR();
...
ppresult_t CToken::PP_Usecom( void )
{
    ...
    word = (char *)s3;
    i = GetToken();
    if ( i != TK_OBJ ) { SetError((char *)"invalid COM symbol name"); return PPRESULT_ERROR; }
    glmode = 0;
    strcase( word );
    if (tstrcmp(word,"global")) {		// global macro
        if ( GetToken() != TK_OBJ ) {
...
ppresult_t CToken::PP_Module( void )
{
    ...
    if ( fl == 0 ) { SetError((char *)"invalid module name"); return PPRESULT_ERROR; }
    if ( !IsGlobalMode() ) { SetError((char *)"not in global mode"); return PPRESULT_ERROR; }
    if ( CheckModuleName( word ) ) {
        SetError((char *)"bad module name"); return PPRESULT_ERROR;
    }
    ...
    if ( PeekToken() != TK_NONE ) {
        ...
        while(1) {
            i = GetToken();
            if ( i != TK_OBJ ) { SetError((char *)"invalid module param"); return PPRESULT_ERROR; }
            AddModuleName( word );
            res = lb->Search(word);
            ...
            if ( wp == NULL ) break;
            if ( i != ',' ) {
                SetError((char *)"invalid module param"); return PPRESULT_ERROR;
            }
            wrtbuf->Put( ',' );
        }
        wrtbuf->PutCR();
...
ppresult_t CToken::PP_Pack( int mode )
{
    ...
    if ( packbuf!=NULL ) {
        i = GetToken();
        if ( i != TK_STRING ) {
            SetError((char *)"invalid pack name"); return PPRESULT_ERROR;
        }
...
ppresult_t CToken::PP_PackOpt( void )
{
    ...
    if ( packbuf!=NULL ) {
        i = GetToken();
        if ( i != TK_OBJ ) {
            SetError((char *)"illegal option name"); return PPRESULT_ERROR;
        }
        strncpy( optname, (char *)s3, 128 );
        i = GetToken();
        if (( i != TK_OBJ )&&( i != TK_NUM )&&( i != TK_STRING )) {
            SetError((char *)"illegal option parameter"); return PPRESULT_ERROR;
        }
...
ppresult_t CToken::PP_RuntimeOpt( void )
{
    ...
    i = GetToken();
    if ( i != TK_STRING ) {
        SetError((char *)"illegal runtime name"); return PPRESULT_ERROR;
    }
    ...
    while(1) {
        ...
        if ( is_preprocess_line ) {
            ...
            if ( res == PPRESULT_INCLUDED ) {
                ...
                RegistExtMacro( (char *)"__file__", fname_literal );
                // ファイル名マクロを更新
                wrtbuf = buf;
