...
void
CToken::CalcCG_token_exprbeg(void)
{
...
void
CToken::CalcCG_token_exprbeg_redo(void)
{
...
static int
is_statement_end(int type)
{
    return (type == TK_SEPARATE) || (type == TK_EOL) || (type == TK_EOF);
}

void
CToken::CalcCG_regmark(int mark)
{
    //		演算子を登録する
    //
    int op;
...
char*
CToken::PickLongStringCG(char* str)
{
    //		指定文字列をmembufへ展開する
    //		( 複数行対応 {"〜"} )
    //
...
char*
CToken::PickStringCG(char* str, int sep)
{
...
char*
CToken::PickStringCG2(char* str, char** strsrc)
{
    //		指定文字列をスキップして終端コードを付加する
    //			sep = 区切り文字
    //
...
char*
CToken::GetTokenCG(char* str, int option)
{
    //		stringデータのタイプと内容を返す
    //		(次のptrを返す)
...
int
CToken::GenerateCodePRMF4(int t)
{
...
void
CToken::GenerateCodeMethod(void)
{
...
void
CToken::GenerateCodeLabel(char* keyname, int ex)
{
    //		HSP3Codeを展開する(ラベル)
    //
    int id, t, i;
...
void
CToken::GenerateCodeVAR(int id, int ex)
{
    //		HSP3Codeを展開する(変数ほか)
    //		(idはlabel ID)
    //
...
void
CToken::CheckCMDIF_Set(int mode)
{
...
void
CToken::CheckCMDIF_Fin(int mode)
{
...
void
CToken::CheckInternalIF(int opt)
{
    //		内蔵命令生成時チェック
    //
    if (opt) { // 'else'+offset
...
void
CToken::CheckInternalListenerCMD(int opt)
{
    //		命令生成時チェック(命令+命令セット)
    //
    int i, t, o;
...
void
CToken::CheckInternalProgCMD(int opt, int orgcs)
{
    //		内蔵プログラム命令生成時チェック
    //
    int i;
...
void
CToken::GenerateCodePP_regcmd(void)
{
...
void
CToken::GenerateCodePP_cmd(void)
{
    //		HSP3Codeを展開する(cmd)
    //
    int id;
...
void
CToken::GenerateCodePP_uselib(void)
{
...
void
CToken::GenerateCodePP_usecom(void)
{
...
void
CToken::GenerateCodePP_func(int deftype)
{
    //		HSP3Codeを展開する(func)
    //
...
void
CToken::GenerateCodePP_comfunc(void)
{
...
int
CToken::GetParameterTypeCG(char* name)
{
...
int
CToken::GetParameterStructTypeCG(char* name)
{
    //		パラメーター名を認識する(struct)
    //
    if (!strcmp(cg_str, "int"))
...
int
CToken::GetParameterFuncTypeCG(char* name)
{
    //		パラメーター名を認識する(func)
    //
    if (!strcmp(cg_str, "int"))
...
int
CToken::GetParameterResTypeCG(char* name)
{
...
void
CToken::GenerateCodePP_deffunc0(int is_command)
{
    ...
    for (i = 0; i < cg_localcur; i++) {
        lb->SetFlag(cg_localstruct[i],
                    -1); // 以前に指定されたパラメーター名を削除する
    }
    cg_localcur = 0;
    funcflag = 0;
    regflag = 1;
...
void
CToken::GenerateCodePP_deffunc(void)
{
    GenerateCodePP_deffunc0(1);
}

void
CToken::GenerateCodePP_defcfunc(void)
{
    GenerateCodePP_deffunc0(0);
}

void
CToken::GenerateCodePP_module(void)
{
...
void
CToken::GenerateCodePP_struct(void)
{
...
void
CToken::GenerateCodePP_defvars(int fixedvalue)
{
...
int
CToken::SetVarsFixed(char* varname, int fixedvalue)
{
    //		変数の固定型を設定する
    //
    int id;
...
void
CToken::GenerateCodePP(char* buf)
{
...
void
CToken::RegisterFuncLabels(void)
{
...
int
CToken::GenerateCodeMain(CMemBuf* buf)
{
    //		ソースをHSP3Codeに展開する
    //		(ソースのバッファを書き換えるので注意)
    //
...
void
CToken::PutCS(int type, int value, int exflg)
{
...
void
CToken::PutCS(int type, double value, int exflg)
{
...
void
CToken::PutCSSymbol(int label_id, int exflag)
{
...
int
CToken::PutDSStr(char* str, bool converts_to_utf8)
{
...
int
CToken::PutDSBuf(char* str, int size)
{
    //		Register strings to data segment (direct)
    //
    int i;
...
void
CToken::PutDI(int dbg_code, int a, int subid)
{
...
int
CToken::PutLIB(int flag, char* name)
{
    int a, i = -1, p;
    LIBDAT lib;
    LIBDAT* l;
...
void
CToken::SetLIBIID(int id, char* clsid)
{
    LIBDAT* l;
    l = (LIBDAT*)li_buf->GetBuffer();
    l += id;
...
int
CToken::PutStructParam(short mptype, int extype)
{
    int size;
    int i;
    STRUCTPRM prm;
...
int
CToken::PutStructEnd(int i, char* name, int libindex, int otindex, int funcflag)
{
...
int
CToken::PutStructEnd(char* name, int libindex, int otindex, int funcflag)
{
    int i = GET_FI_SIZE();
    fi_buf->PreparePtr(sizeof(STRUCTDAT));
...
int
CToken::PutStructEndDll(char* name, int libindex, int subid, int otindex)
{
    //		STRUCTDATを登録する(DLL用)
    //
    int i;
...
void
CToken::PutHPI(short flag, short option, char* libname, char* funcname)
{
    HPIDAT hpi;
    hpi.flag = flag;
    hpi.option = option;
...
int
CToken::GenerateCode(char* fname, char* oname, int mode)
{
    CMemBuf srcbuf;
...
int
CToken::GenerateCode(CMemBuf* srcbuf, char* oname, int mode)
{
    //		ファイルをHSP3Codeに展開する
...
void
CToken::CG_MesLabelDefinition(int label_id)
{
    if (!cg_debug)
        return;
