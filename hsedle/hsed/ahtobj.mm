...
//#import "ahtobj.h"
//
//@implementation CAht : NSObject
//- (instancetype)init
//{
//    self = [super init];
//    if (self) {
//        stdbuf = new CMemBuf( 0x1000 );        // debug message buffer
//        [self Mesf:(char *)"AHT processor ready.¥r¥n"];
...
//    strncpy( prjdir, name, _FNMAX-1 );
//}
//-(void)SetPrjFile:(char*)name {
//    strncpy( fname, name, _FNMAX-1 );
//}
//-(void)SetToolDir:(char*)name {
//    strncpy( tooldir, name, _FNMAX-1 );
//}
//-(int)GetModelCount {
//    return model_cnt;
//}
...
//    [self DisposeModel];
//    dirinfo( p, 0x10005 );
//    [self SetPrjDir:p];						// マイドキュメントを設定
//    dirinfo( p, 1 );
//    [self SetToolDir:p];					// 自分のディレクトリ
//    [self SetPrjFile:(char *)"ahttmp"];
//    [self SetPage:0 max:0];
//}
//-(void)DisposeModel {
//    int i;
...
//            return [self EntryModel:i];
//        }
//    }
//    return [self EntryModel:model_cnt];
//}
...
//    }
//    if ( mem_ahtmodel[ i ] != NULL ) [self DeleteModel:i];
//    model = new AHTMODEL;
//    model->SetId( i );
...
//                myid = [self BuildGlobalIDSub:model->GetSource() pname:model->GetSourcePath() i:i];
...
//                res = p->newval;
//                if ( res == NULL ) {
//                    list = [self SearchModelByClassName:p->defval2];
//                    note.Select( list );
...
//    obj = [self GetProjectFileObject:fileid];
//    model->SetObj( (AHTOBJ *)[self GetProjectFileString:obj->ahtobj] );
//    obj++;
//    //			プロパティ全設定
//    while(1) {
//        if ( obj->ahtsource != -1 ) break;
//        //Alertf( "%s=%s", GetProjectFileString( obj->propname), GetProjectFileString( obj->defvalue )  );
//        prop = model->GetProperty( [self GetProjectFileString:obj->propname] );
//        if ( prop != NULL ) {
//            prop->SetNewVal( [self GetProjectFileString:obj->defvalue] );
//        }
...
//    HTPOBJ *obj;
//    obj = [self GetProjectFileObject:id];
//    return [self GetProjectFileString:obj->ahtsource];
//}
//-(char*)GetProjectFileModelPath:(int)id {
//    HTPOBJ *obj;
//    obj = [self GetProjectFileObject:id];
//    return [self GetProjectFileString:obj->propname];
//}
...
//    if ( mem_ahtmodel_size ) {
//        for(i=0;i<model_cnt;i++) {
//            [self SaveProjectSub:mem_ahtmodel[i]];
//        }
//    }
//    //	Output file
//    //
...
//        [self BuildPartsSub:i fname:fullpath];
//    }
//    return maxparts;
//}
...
//    return [self GetParts:id]->name;
//}
//-(char*)GetPartsClassName:(int)id {
//    return [self GetParts:id]->classname;
//}
//-(int)GetPartsIconID:(int)id {
//    return [self GetParts:id]->icon;
//}
//-(void)InitMakeBuffer {
//    [self DisposeMakeBuffer];
...
//    return 0;
//}
//-(void)AddMakeBufferInit:(char*)str size:(int)size {
...
//    ahtini_buf->PutCR();
//}
//-(void)AddMakeBufferMain:(char*)str size:(int)size {
//    //		スクリプトバッファに追加
//    //
//    if ( size <= 0 ) {
