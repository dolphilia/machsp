...
//        uint8 c = a1;
//        //if (c >= 0 && c < 128) { return 1; }
//        if (c >= 128 && c < 192) { if (a<OBJNAME_MAX){s2[a++]=a1; vs++; a1=*vs; s2[a++]=a1; vs++;} else {vs+=2;} continue; }
//        if (c >= 192 && c < 224) { if (a<OBJNAME_MAX){s2[a++]=a1; vs++; a1=*vs; s2[a++]=a1; vs++;} else {vs+=2;} continue; }
//        if (c >= 224 && c < 240) { if (a<OBJNAME_MAX){s2[a++]=a1; vs++; a1=*vs; s2[a++]=a1; vs++;} else {vs+=2;} continue; }
//        if (c >= 240 && c < 248) { if (a<OBJNAME_MAX){s2[a++]=a1; vs++; a1=*vs; s2[a++]=a1; vs++;} else {vs+=2;} continue; }
//        if (c >= 248 && c < 252) { if (a<OBJNAME_MAX){s2[a++]=a1; vs++; a1=*vs; s2[a++]=a1; vs++;} else {vs+=2;} continue; }
//        if (c >= 252 && c < 254) { if (a<OBJNAME_MAX){s2[a++]=a1; vs++; a1=*vs; s2[a++]=a1; vs++;} else {vs+=2;} continue; }
//        if (c >= 254 && c <= 255) { if (a<OBJNAME_MAX){s2[a++]=a1; vs++; a1=*vs; s2[a++]=a1; vs++;} else {vs+=2;} continue; }
...
void CToken::GenerateCodePP_func( int deftype )
{
    ...
    if ( ref == 0 && (otflag & STRUCTDAT_OT_CLEANUP) == 0 ) {
        if ( hed_cmpmode & CMPMODE_OPTINFO ) {
            ...
            @autoreleasepool {
                AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
                global.logString = [global.logString stringByAppendingFormat:@"#未使用の外部DLL関数の登録を削除しました %s\n", fbase];
            }
        ...
        if ( i >= 0 ) {
            ...
            if ( ref == 0 ) {
                ...
                if ( hed_cmpmode & CMPMODE_OPTINFO ) {
                    ...
                    @autoreleasepool {
                        AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
                        global.logString = [global.logString stringByAppendingFormat:@"#未使用のモジュールを削除しました %s\n", modname];
                    }
#else
...
//        if (c >= 224 && c < 240) { p++;p++; if ( *p >= 32 ) { p++; continue; } }
//        if (c >= 240 && c < 248) { p++;p++;p++; if ( *p >= 32 ) { p++; continue; } }
//        if (c >= 248 && c < 252) { p++;p++;p++;p++; if ( *p >= 32 ) { p++; continue; } }
//        if (c >= 252 && c < 254) { p++;p++;p++;p++;p++; if ( *p >= 32 ) { p++; continue; } }
//        if (c >= 254 && c <= 255) { p++;p++;p++;p++;p++;  if ( *p >= 32 ) { p++; continue; } }
        //
        if ( a1 == 0 ) break;
        if ( a1 == 13 ) {
            *p = 0; p++; line++;
    ...
    try {
        ...
        for( a=0; a<lb->GetCount(); a++ ) {
            if ( lb->GetType(a) == TYPE_XLABEL ) {
                ...
                @autoreleasepool {
                    AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
                    global.logString = [global.logString stringByAppendingFormat:@"#ラベルの定義が存在しません [%s]\n", lb->GetName(a)];
                }
#else
...
#ifdef JPNMSG
                //Mesf( (char *)"#関数が定義されていません [%s]", lb->GetName(GET_FI(a)->otindex) );
                @autoreleasepool {
                    AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
                    global.logString = [global.logString stringByAppendingFormat:@"#関数が定義されていません [%s]\n", lb->GetName(GET_FI(a)->otindex)];
                }
#else
        ...
        if ( iflev > 0 ) {
            ...
            @autoreleasepool {
                AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
                global.logString = [global.logString stringByAppendingString:@"#波括弧が閉じられていません\n"];
            }
#else
...
int CToken::GenerateCode( char *fname, char *oname, int mode )
{
    ...
    if ( srcbuf.PutFile( fname ) < 0 ) {
        ...
        @autoreleasepool {
            AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
            global.logString = [global.logString stringByAppendingString:@"#No file.\n"];
        }
        return -1;
    }
...
int CToken::GenerateCode( CMemBuf *srcbuf, char *oname, int mode )
{
    ...
    if ( res ) {
        ...
        CStrNote note;
        CMemBuf srctmp;
        //Mesf( (char *)"%s(%d) : error %d : %s (%d行目)", cg_orgfile, cg_orgline, res, cg_geterror((CGERROR)res), cg_orgline );
        ...
        @autoreleasepool {
            NSString* nsstr_error_message = [NSString stringWithCString:err[(int)res] encoding:NSUTF8StringEncoding];
            //NSLog(@"%@",nsstr_error_message);
            
            AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
            global.logString = [global.logString stringByAppendingFormat:@"%s(%d) : error %d : %@ (%d行目)\n", cg_orgfile, cg_orgline, res, nsstr_error_message, cg_orgline];
            ...
            if ( cg_errline > 0 ) {
                ...
                [alert setInformativeText:[NSString stringWithFormat:@"error %d : %@ (%d行目)\n--> %s\n", res, nsstr_error_message, cg_orgline,tmp]];
                ...
                NSAlert *alert = [[NSAlert alloc] init];
                [alert setMessageText:@"コンパイルエラー"];
                [alert setInformativeText:[NSString stringWithFormat:@"error %d : %@ (%d行目)\n", res, nsstr_error_message, cg_orgline]];
                [alert addButtonWithTitle:@"OK"];
                [alert runModal];
            }
        ...
        if ( res<0 ) {
            ...
            @autoreleasepool {
                AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
                global.logString = [global.logString stringByAppendingString:@"#出力ファイルを書き込めません\n"];
            }
#else
            ...
            @autoreleasepool {
                AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
                global.logString = [global.logString stringByAppendingFormat:@"# Code size (%d) String data size (%d) param size (%d)\n",cs_size,ds_size,mi_buf->GetSize() ];
                global.logString = [global.logString stringByAppendingFormat:@"# Vars (%d) Labels (%d) Modules (%d) Libs (%d) Plugins (%d)\n",cg_valcnt,ot_size>>2,n_mod,li_size,n_hpi ];
                global.logString = [global.logString stringByAppendingFormat:@"# No error detected. (total %d bytes)\n",hsphed.allsize ];
                global.logString = [global.logString stringByAppendingString:@"\n"];
            }
            
            
            
            //Mesf( (char *)"#Code size (%d) String data size (%d) param size (%d)",cs_size,ds_size,mi_buf->GetSize() );
            //Mesf( (char *)"#Vars (%d) Labels (%d) Modules (%d) Libs (%d) Plugins (%d)",cg_valcnt,ot_size>>2,n_mod,li_size,n_hpi );
...
void CToken::CG_MesLabelDefinition(int label_id)
{
    ...
    if ( labobj->def_file ) {
#ifdef JPNMSG
        //Mesf((char *)"#識別子「%s」の定義位置: line %d in [%s]", lb->GetName(label_id), labobj->def_line, labobj->def_file);
        @autoreleasepool {
            AppDelegate* global = (AppDelegate *)[[NSApplication sharedApplication] delegate];
            global.logString = [global.logString stringByAppendingFormat:@"#識別子「%s」の定義位置: line %d in [%s]\n", lb->GetName(label_id), labobj->def_line, labobj->def_file];
        }
#else
