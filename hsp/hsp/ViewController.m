...
- (void)viewDidLoad
{
    ...
    if (isExists && isDir == NO) { // start.axがリソースに存在する
        ...
        path = [[[[NSBundle mainBundle] bundlePath]
                 stringByDeletingLastPathComponent] stringByAppendingString:@"/start.ax"];
        isExists = [filemanager fileExistsAtPath:path isDirectory:&isDir];
    ...
    for (int i = 0; i < 64; i++) {
        [myButtons
         addObject:[[NSButton alloc]
                    initWithFrame:
                    NSMakeRect(0, -9999, 81,
                               32)]]; //あらかじめ画面の表示範囲外に設置しておく
    ...
    for (int i = 0; i < 64; i++) {
        [mySliders
         addObject:[[NSSlider alloc] initWithFrame:NSMakeRect(0, -9999, 96, 21)]];
    ...
    for (int i = 0; i < 64; i++) {
        [myCheckBoxs
         addObject:[[NSButton alloc] initWithFrame:NSMakeRect(0, -9999, 100, 18)]];
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.001
                                     target:self
                                   selector:@selector(onTimerMidiEvent:)
                                   userInfo:nil
                                    repeats:YES];
