...
void getpath( const char *src, char *outbuf, int flag ) {
    char *p;
    char stmp[_MAX_PATH];
...
/// ファイル名に拡張子を追加する
/// @param str 元の文字列
/// @param ext 連結する文字列（拡張子）
void addext(char *str, const char *ext) {
    int index = 0;
    while(1) {
        if (str[index] == 0)
            break;
...
/// 文字列から最後にマッチした文字の位置を取得する
///
/// (全角対応版)
/// @param str char* 対象の文字列
/// @param code char 探したい文字
char *strchr2(char *str, char code) {
    unsigned char *buf = (unsigned char*)str;
    char *res = NULL;
    while(1) {
        if (*buf == 0) //文字列の終端
...
/// @param str 文字列のポインタ
/// @param code 不要
void CutLastChr(char *str, char code) {
    char *buf1 = strchr2(str, '/');
    char *buf2;
    if (buf1 != NULL) {
