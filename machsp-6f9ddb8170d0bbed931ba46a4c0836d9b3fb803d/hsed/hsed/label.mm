...
int
CLabel::StrCmp(char* str1, char* str2)
{
...
int
CLabel::Regist(char* name, int type, int opt)
{
    return Regist(name, type, opt, NULL, -1);
}

int
CLabel::Regist(char* name, int type, int opt, char const* filename, int line)
{
    if (name[0] == 0)
        return -1;
...
void
CLabel::SetData(int id, char* str)
{
    //		set data
    //
...
void
CLabel::SetData2(int id, char* str, int size)
{
    //		set data
    //
    LABOBJ* lab = &mem_lab[id];
...
void
CLabel::SetInitFlag(int id, int val)
{
    //		set init flag
    //
...
void
CLabel::SetForceType(int id, int val)
{
    //		set force type
    //
...
CLabel::Search(char* oname)
{
    ...
    StrCase(oname);
    if (*oname != 0) {
        std::pair<LabelMap::iterator, LabelMap::iterator> r =
        labels.equal_range(oname);
...
int
CLabel::SearchLocal(char* oname, char* loname)
{
    ...
    if (*oname != 0) {
        ...
        std::pair<LabelMap::iterator, LabelMap::iterator> r2 =
        labels.equal_range(loname);
...
void
CLabel::DisposeSymbolBuffer(void)
{
    int i;
    for (i = 0; i < def_maxblock; i++) {
        if (symblock[i] != NULL) {
...
char*
CLabel::ExpandSymbolBuffer(int size)
{
    char* p;
...
int
CLabel::GetCount(void)
{
    return cur;
}

int
CLabel::GetSymbolSize(void)
{
    return ((maxsymbol * (curblock - 1)) + symcur);
}

int
CLabel::GetFlag(int id)
{
    return mem_lab[id].flag;
}

int
CLabel::GetOpt(int id)
{
    return mem_lab[id].opt;
}

int
CLabel::GetType(int id)
{
    return mem_lab[id].type;
}

char*
CLabel::GetName(int id)
{
    return mem_lab[id].name;
}

char*
CLabel::GetData(int id)
{
    return mem_lab[id].data;
}

char*
CLabel::GetData2(int id)
{
...
int
CLabel::GetInitFlag(int id)
{
    return (int)mem_lab[id].init;
}

char*
CLabel::RegistSymbol(char* str)
{
    //		シンボルテーブルに文字列を登録
    //
    char* p;
...
char*
CLabel::RegistTable(char* str, int size)
{
    //		シンボルテーブルにテーブルデータを登録
    //
    char* p;
...
int
CLabel::RegistList(char** list, char* modname)
{
    //		キーワードリストを登録する
    //
    char* p;
...
int
CLabel::RegistList2(char** list, char* modname)
{
...
CLabel::DumpLabel(char* str)
{
    ...
    for (a = 0; a < cur; a++) {
        ...
        sprintf(tmp, "#ID:%d (%s) flag:%d  type:%d  opt:%x", a, lab->name,
                lab->flag, lab->type, lab->opt);
        p = Prt(p, tmp);
        //		lab = GetLabel( Search( lab->name) );
        //		sprintf( tmp,"#ID:%d (%s) flag:%d  type:%d
        //opt:%x",a,lab->name,lab->flag,lab->type,lab->opt );
        //		p=Prt( p,tmp );
    }
    *p = 0;
}
...
void
CLabel::DumpHSPLabel(char* str, int option, int maxsize)
{
    char tmp[256];
    char* typem;
    //	char optm[256];
...
int
CLabel::SearchRelation(int id, int rel_id)
{
...
void
CLabel::AddRelation(int id, int rel_id)
{
    //		ラベル依存のIDデータを追加する
    //
    LABREL* rel;
...
void
CLabel::AddRelation(char* name, int rel_id)
{
    int i;
    i = Search(name);
    if (i < 0)
...
void
CLabel::SetCaseMode(int flag)
{
    casemode = flag;
}

void
CLabel::SetDefinition(int id, char const* filename, int line)
{
