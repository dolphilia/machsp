...
#include "../util/utility_stack.h"
//#import "../gui/AppDelegate.h"
//#import "c_wrapper.h"
...
CToken::CToken(void) {
    ...
    undefined_symbols = (undefined_symbol_t *) vector_create();
...
//#define __vector_add(vec_addr, value) (*__vector_add_asg(vec_addr) = value)
    ...
    if (wrtbuf != NULL) { // 登録されていないキーワードを展開
        if (strcmp((char *) s2, fixname)) {
            ...
            vector v = _vector_add((vector *) &undefined_symbols, sizeof(**&undefined_symbols));
            (*((__typeof__(*&undefined_symbols)) (v)) = sym);
    ...
    *dummy = 0;
    ...
    while (1) { // マクロ置き換え
        ...
        if (cur_char == '%') {
            ...
            type = GetToken();
            if (type == TOKEN_OBJ) { // 特殊コマンドラベル処理
                macbuf = mactmp;
                *mactmp = 0;
                cur_char = tolower((int) *s3);
                switch (cur_char) {
                    case 't': // %tタグ名
                        tagid = GetTagID((char *) (s3 + 1));
                        //tagid = tstack->GetTagID((char *)(s3 + 1));
                        break;
                    case 'i':
                        GetTagUniqueName(tagid, mactmp);
                        PushTag(tagid, mactmp);
                    ...
                    case 'p':
                        ...
                        if ((stklevel < 0) || (stklevel > 9))
                            stklevel = 0;
                        macbuf = LookupTag(tagid, stklevel);
                ...
                if (macbuf == NULL) {
                    sprintf(mactmp, "macro syntax error [%s]", GetTagName(tagid));
...
ppresult_t CToken::PP_Module(void) {
    ...
    if ((token == TOKEN_OBJ) || (token == TOKEN_STRING))
        fl = 1;
    if (token == TOKEN_NONE) {
...
ppresult_t CToken::PP_Aht(void) {
    ...
    token = GetToken();
    if ((token != TOKEN_STRING) && (token != TOKEN_NUM)) {
...
ppresult_t CToken::PP_Ahtmes(void) {
    ...
    while (1) {
        ...
        if (token == TOKEN_NONE)
            break;
        if ((token != TOKEN_OBJ) && (token != TOKEN_NUM) && (token != TOKEN_STRING)) {
...
void CToken::FinishPreprocess(CMemBuf *buf) {
    ...
    size_t len = vector_size(undefined_symbols);
